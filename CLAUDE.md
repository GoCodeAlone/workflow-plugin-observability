# CLAUDE.md — Workflow Plugin Observability
...
```sh
GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -ldflags="-s -w" -o workflow-plugin-observability ./cmd/workflow-plugin-observability/
```

## Structure

- `cmd/workflow-plugin-observability/main.go` — Plugin entry point (calls `sdk.Serve`)
